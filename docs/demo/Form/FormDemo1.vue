...
const rules = {
  ...
  email: {
    ...
    validator: (value: any) => {
      if (formData.value.contactType === 'email' && !value) return 'Email is required';
  ...
  phone: {
    ...
    validator: (value: any) => {
      if (formData.value.contactType === 'phone' && !value) return 'Phone number is required';
...
const handleCurrentToggle = (index: number) => {
  formData.value.experience[index].current = !formData.value.experience[index].current;
...
<template>
  <!-- <Demo> -->
    <Card title="动态表单示例" :border="false" style="padding: 20px;">
      <Form v-model="formData" :rules="rules" @submit="handleSubmit" labelAlign="left" labelJustifyContent="left" labelWidth="100px">
        ...
        <FormItem label="Contact" prop="contactType" required>
          <Radio v-model="formData.contactType" :options="[{ label: 'Email', value: 'email' }, { label: 'Phone', value: 'phone' }]" />
        </FormItem>

        <FormItem v-if="formData.contactType === 'email'" label="Email" prop="email" required>
        ...
        <FormItem label="Address" prop="address">
          <Textarea v-model="formData.address" placeholder="Enter your address" :rows="3" />
        </FormItem>

        <FormItem label="Skills" style="margin-bottom: 0;">
          <div v-for="(skill, index) in formData.skills" :key="index" style="margin-bottom: 10px; display: flex; align-items: center;">
            <Input
          ...
          <Button @click="addSkill" type="default" border size="small" style="margin-top: 5px;">
            Add Skill
          </Button>
        </FormItem>

        <FormItem label="Experience" style="margin-bottom: 0;">
          <div v-for="(exp, index) in formData.experience" :key="index" style="margin-bottom: 20px; padding: 15px; background-color: var(--color-background-soft); border-radius: var(--border-radius);">
            <h4 style="margin-top: 0; margin-bottom: 15px;">Experience {{ index + 1 }}</h4>
            ...
            <FormItem label="" prop="current" style="margin-bottom: 0;">
              <div style="display: flex; align-items: center;">
                <Switch v-model="exp.current" @change="handleCurrentToggle(index)" />
          ...
          <Button @click="addExperience" type="default" border size="small" style="margin-top: 5px;">
            Add Experience
          </Button>
        </FormItem>
