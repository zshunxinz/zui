...
<style>
    .input-group .z-input { margin-bottom: 10px; }
</style>
...
```vue
<template>
  <Input v-model="value" placeholder="点击输入框" @click="handleClick" />
</template>
...
<template>
  <div class="input-group">
    <Input
      ...
      v-model="text"
      autosize
      placeholder="自适应高度文本框"
    />
    <Input
      type="textarea"
      v-model="text2"
      :autosize="{ minRows: 3, maxRows: 5 }"
      placeholder="自定义最小最大行数"
    />
  </div>
</template>
...
| tabindex        | string           | -      | 输入框的 tabindex                                                                |
| auto-complete   | string           | off    | 下个主版本弃用，请使用 autocomplete                                              |
...
| click       | 点击输入框时触发     | -                          |
| enter       | 按下 Enter 键时触发  | (value: string) 当前输入值 |
| clear       | 点击清空按钮时触发   | -                          |
